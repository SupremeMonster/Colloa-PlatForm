	...
	return {
		...
		contents: [{
			...
			elements: [{
				...
				commit: function(a) {
					a = a.element;
					this.getValue() ? (a.data("cke-saved-name", this.getValue()),a.setAttribute('id', this.getValue())) : (a.data("cke-saved-name", !1), a.removeAttribute("name"),a.removeAttribute("id"))
				}
			}, {
				id: "value",
				type: "text",
				...
				commit: function(a) {
					...
					if (CKEDITOR.env.ie) {
						var d = c.getAttribute("checked"),
							e = !! this.getValue();
						d != e && (d = CKEDITOR.dom.element.createFromHtml('<input type="radio"' + (e ? ' checked="checked"' : "") + "></input>", b.document), c.copyAttributes(d, {
							type: 1,
							checked: 1
