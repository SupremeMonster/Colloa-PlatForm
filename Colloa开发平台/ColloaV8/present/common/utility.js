...
function _$(arg) {
  ...
  if (m_element){
    if (m_element.tagName == "INPUT") {
      if (m_element.type == "radio") {
        m_element = document.getElementsByName(m_element.name);
  ...
  this.value = function (arg1, arg2) {
    ...
    if (arg1 != null) {
      ...
      if (m_flag > 0) {
        for (var i = 0; i < m_element.length; i++) {
          if (m_element[i].checked) return m_element[i].value;
        }
        return m_element[0].value;
      }
      else if (m_flag == 0) {
        var s = m_element.value.replace("<br>","").replace(/&nbsp;/g, " ").replace(/\u00a0/g," ").replace(/(^\s*)|(\s*$)/g, "");
  ...
  this.attribute = function (arg1, arg2) {
    ...
    if (arg2 != null) {
      if(m_id.indexOf('e.')==0){
        m_element.parentNode.setAttribute(arg1, arg2);
      }
      else{
      ...
      if(m_id.indexOf('e.')==0){
        return m_element.parentNode.getAttribute(arg1);
      }else{
        return m_element.getAttribute(arg1);
      }
    }
  }
          ...
          for(var i=0; i<aFields.length; i++) {
            ...
            if(sField.indexOf("=") > 0) {
              ...
              while(n >= 0) {
                n2 = s.indexOf("]", n);
                sTemp = s.substring(n+2,n2);
                if((nTemp=sTemp.indexOf("."))>0) {
                  if(sTemp.substr(0,nTemp)=="-1") sTemp = oList[iLoopRow].getAttribute(sTemp.substr(nTemp+1));
                  else if(sTemp.substr(nTemp+1)=="innerText") sTemp=(oTDs[sTemp.substr(0,nTemp)].innerText||oTDs[sTemp.substr(0,nTemp)].textContent);
                  else sTemp = oTDs[sTemp.substr(0,nTemp)].getAttribute(sTemp.substr(nTemp+1));
                }
...
function $dateFormat(format, dateValue) { /* eg: format="yyyy/MM/dd hh:mm:ss" */
  var d = (dateValue instanceof Date) ? dateValue : typeof(dateValue)=="object" ? new Date(dateValue.innerHTML.replace(/-/g,"/")) : typeof(dateValue)=="string" ? new Date(dateValue.replace(/-/g,"/")) : new Date();
  var o = {
...
function $table2excel(sTableIdOrFileName, sSql) {
  if(sSql) { //Build the Excel content on Server to download
    //windowOpen("../view/file.aspx?attachment=1&name="+escape(sTableIdOrFileName)+".xls&contentType=application/vnd.ms-excel&file="+escape(sSql));
    var oForm = document.getElementById("_formOfTable2excel");
    if(!oForm) {
      oForm = document.createElement("form");
      oForm.id = "_formOfTable2excel";
      oForm.target = "_blank";
      oForm.method = "post";
      oForm.style.display = "none";
      var oInput = document.createElement("input");
      oInput.type = "hidden";
      oInput.name = "sqlForFile";
      oForm.appendChild(oInput);
      document.body.appendChild(oForm);
    }
    oForm.elements[0].value=sSql;
    oForm.action = "../view/file.aspx?attachment=1&name="+escape(sTableIdOrFileName)+".xls&contentType=application/vnd.ms-excel";
    oForm.submit();
  }
...
function $tableEval(sTableId) {
  var n, n2, k, sRow, sCol, sTemp, sEval, sEval2;
  var trs = document.getElementById(sTableId).rows;
  for(var i=1; i<trs.length; i++) {
    var tds = trs[i].cells;
    for(var j=1; j<tds.length; j++) {
      sEval = tds[j].getAttribute("dbf.eval");
      if(!sEval) sEval = trs[0].cells[j].getAttribute("dbf.eval");;
      if(!sEval) sEval = trs[i].cells[0].getAttribute("dbf.eval");;
      if(!sEval) continue;
      if(sEval.indexOf("!") == 0) { // "!'r1c1'+'r2c1'+'-test-cc-'+'c0'"; "!r1c1+r2c1";
        sEval = sEval.substr(1); sEval2 = "";
        n = 0; n2 = sEval.indexOf("r",n);
        while(n2 >= 0) {
          sEval2 += sEval.substr(n,n2-n).replace(/c/g,"!");
          sRow = sEval.substr(n2+1,1);
          if(!(/^-?\d+$/.test(sRow))) { n = n2; n2 = sEval.indexOf("r",n+1); continue; }
          k=2; for(; k<6; k++) { sTemp = sEval.substr(n2+k,1); if(!(/^-?\d+$/.test(sTemp))) break; sRow += sTemp; }
          if(sTemp == "c") {
            sCol = sEval.substr(n2+k+1, 1);
            if(!(/^-?\d+$/.test(sCol))) { n = n2; n2 = sEval.indexOf("r",n+1); continue; }
            k+=2; for(; k<10; k++) { sTemp = sEval.substr(n2+k,1); if(!(/^-?\d+$/.test(sTemp))) break; sCol += sTemp; }
          }
          else sCol = j;
          if(Number(sRow)<trs.length && Number(sCol)<tds.length) {
            sEval2 += $(trs[sRow].cells[sCol]).value();
            n = n2+k; n2 = sEval.indexOf("r",n+1);
          }
          else n2 = -10;
        }
        if(n2 == -10) { $(tds[j]).value("?r"+sRow+"c"+sCol); continue; }
        sEval2 += sEval.substr(n).replace(/c/g,String.fromCharCode(22));

        sEval = sEval2; sEval2 = "";
        n = 0; n2 = sEval.indexOf(String.fromCharCode(22),n);
        while(n2 >= 0) {
          sEval2 += sEval.substr(n,n2-n);
          sCol = sEval.substr(n2+1,1);
          if(!(/^-?\d+$/.test(sCol))) { n = n2; n2 = sEval.indexOf(String.fromCharCode(22),n+1); continue; }
          k=2; for(; k<6; k++) { sTemp = sEval.substr(n2+k,1); if(!(/^-?\d+$/.test(sTemp))) break; sCol += sTemp; }
          if(Number(sCol)<tds.length) {
            sEval2 += $(trs[i].cells[sCol]).value();
            n = n2+k; n2 = sEval.indexOf(String.fromCharCode(22),n+1);
          }
          else n2 = -10;
        }
        if(n2 == -10) { $(tds[j]).value("?c"+sCol); continue; }
        sEval2 += sEval.substr(n);
        sEval2 = sEval2.replace(new RegExp(String.fromCharCode(22),"g"),"c");
        //$(tds[j]).value(sEval2); continue; //for test
        try {
          sTemp = tds[j].getAttribute("dbf.type");
          if(sTemp=="number") $(tds[j]).number(eval(sEval2));
          else if(sTemp=="amount") $(tds[j]).amount(eval(sEval2));
          else $(tds[j]).value(eval(sEval2));
        }
        catch(e) { $(tds[j]).value("NaN"); }
      }
      else {
        try { $(tds[j]).value(eval(sEval2)); }
        catch(e) { $(tds[j]).value("NaN"); }
      }
    }
  }
  return 1; //return 0 for test
}
...
//for legacy support, to be removed
function $form2doc(docPath, docName, loopAppended, loopAppendedByRow, flag) {
  if(!docName || docPath.indexOf("\"")>=0 || docName.indexOf("\"")>=0)
  {
    alert("[form2doc] invalid path or name");
    return 0;
  }
  if(flag) {
    if(!confirm("[form2doc] " + translate["areYouSure"])) {
      return 0;
    }
  }
  var xmlHttp;
  ...
  if (xmlHttp) {
    ...
    xmlHttp.open("POST", "invoke.aspx?key=form2doc&context="+_nContextId+"&"+Math.random(), false);
    xmlHttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    var s="";
    if(loopAppendedByRow) {s="<tr>";var aLoop=loopAppendedByRow.split(",");for(var i=0;i<aLoop.length;i++){if(aLoop[i].indexOf("[!")>=0) s+="<td>"+aLoop[i]+"</td>";else{s+="<td>"+$(aLoop[i]).value().replace(/\r\n/g,"<br>")+"</td>";if(flag) $(aLoop[i]).value('');}}s+="</tr>";}
    else if(loopAppended) s=loopAppended;
    xmlHttp.send("docPath="+encodeURIComponent(docPath)+"&docName="+encodeURIComponent(docName)+"&loopAppended="+encodeURIComponent(s));
    if (xmlHttp.responseText.length > 0) {
      alert("[form2doc] " + xmlHttp.responseText);
      return 0;
    }
    else {
      return 1;
